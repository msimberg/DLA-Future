...
DLAF_MAKE_GPUBLAS_OP(Trmm3, trmm);
...
DLAF_MAKE_GPUBLAS_OP(Trmm3, trmm);
#elif HIP_VERSION < 50700000
DLAF_MAKE_GPUBLAS_OP(Trmm3, trmm_outofplace);
#else
DLAF_MAKE_GPUBLAS_OP(Trmm3, trmm);
#endif
...
#if defined(DLAF_WITH_CUDA) || (defined(DLAF_WITH_HIP) && HIP_VERSION >= 50700000)
...
void trmm3(cublasHandle_t handle, const blas::Side side, const blas::Uplo uplo, const blas::Op op,
  ...
  gpublas::internal::Trmm3<T>::call(handle, blasToCublas(side), blasToCublas(uplo), blasToCublas(op),
                                    blasToCublas(diag), to_int(s.m), to_int(s.n),
                                    blasToCublasCast(&alpha), blasToCublasCast(a.ptr()), to_int(a.ld()),
