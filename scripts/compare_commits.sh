...
for configuration in "${configurations[@]}"; do
    ...
    for commitish in "$@"; do
        ...
        spec_hash=$(${spack_cmd} spec --json ${spec} | jq --raw-output '.spec.nodes[] | select(.name == "dla-future") | .hash')
        ...
        ${spack_cmd} spec -lI ${spec}
        # install dependencies
        ${spack_cmd} install --only dependencies ${spec}

        # build dla-future
        ${spack_cmd} build-env ${spec} -- cmake ${worktree_path} -B${build_dir} ${cmake_flags}
        ${spack_cmd} build-env ${spec} -- cmake --build ${build_dir} --target miniapp_{cholesky,triangular_solver,gen_to_std}
