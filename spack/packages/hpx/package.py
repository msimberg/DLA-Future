...
class Hpx(CMakePackage, CudaPackage, ROCmPackage):
    ...
    version('no-locks-polling', git='https://github.com/msimberg/hpx.git', tag='no-locks-polling')
    ...
    depends_on('asio', when='@pch')
    depends_on('asio', when='@master')
    depends_on('asio', when='@stable')
    depends_on('asio', when='@no-locks-polling')
    for cxxstd in cxxstds:
        depends_on(
    ...
    #depends_on('apex', when='instrumentation=apex_minimal')
    #depends_on('tracy', when='instrumentation=tracy')
    ...
    #with when('@:1.8'):
    #    conflicts('instrumentation=tracy')
    ...
    def cmake_args(self):
        ...
        # Tracy is not installed or found using normal CMake mechanisms. We
        # inform HPX about the root of the custom install.
        if 'instrumentation=tracy' in spec:
            args += [ self.define('TRACY_ROOT', spec['tracy'].prefix) ]
