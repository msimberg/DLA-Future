...
using namespace dlaf::internal::gpu;
...
template <typename T>
void print_tile(Tile<T, Device::CPU> const& t) {
  auto s = t.size();
  auto n = s.cols();
  auto m = s.rows();
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      std::cout << t(TileElementIndex{i, j}) << " ";
    }
    std::cout << std::endl;
  }
}
...
void testTrsm(blas::Side side, blas::Uplo uplo, blas::Op op, blas::Diag diag, SizeType m, SizeType n,
  ...
  cublas_trsm(handle, side, uplo, op, diag, alpha, ad, bd);
